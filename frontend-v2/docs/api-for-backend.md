...
| via | 動き | 返すもの |
|---|---|---|
| `qr` | 直接会って読み取ったので、**すぐにフレンド** | `201 { "status": "friends", "user": { "userId", "displayName" } }` |
| `link` | リンク・キー入力。**相手の承認が必要** | `202 { "status": "requested", "requestId", "user": {…} }` |

- `link` で、相手からもこちらに申請が来ていた場合は、その場でフレンドにして201
- エラー：`404 share_key_not_found`、`400 self`（自分のキー）、`409 already_friends`、`409 blocked_by_you`（自分がブロック中）
...
1. **アプリを消して入れ直したとき**：ログインがないので端末トークンが消え、同じMACアドレスで登録しようとすると `409 mac_taken` になる。引き継ぎ方法（例：登録時に復元用コードを表示する）を決める必要がある
